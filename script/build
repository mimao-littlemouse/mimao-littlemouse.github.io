...
# 生产环境下的构建jekyll，只是构建，不会启动服务）
# 设定 指定读取开发环境下的配置文件 启用安全模式（可在_config中配置，这里默认使用命令行配置）
# 还可指定的选项有：
...
JEKYLL_ENV=${jekyll_env} JEKYLL_GITHUB_TOKEN=${github_token} bundle exec jekyll build --config _config.yml